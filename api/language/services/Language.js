...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Language.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Language.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Language._attributes)
      .filter(attribute => attribute !== Language.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Language._attributes[attribute].type));
